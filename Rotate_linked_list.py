...
Can k be larger than the linked list? Is it a singly linked list
or can it be a circular linked list?

What reasonable assumptions could you state?

k can be larger than the length of the list, normal singly linked list

What are 2-3 ways you can simplify the problem?

None it's already simple enough.

Brainstorm 2-3 ways to approach the problem.

Create new linked list, then add nodes starting at k index until end,
then add nodes starting from the head until k index

Change head to be k index, break link from k-1 index if it exists,
then change current tail.next to be the previous head, and k-1 to tail

Choose one idea and write pseudocode for it.

2nd approach:

Move to node k % len(list) while tracking k-1 node
Make tail's next the head of list
Make node k the new head
Make node k-1 new tail and break link from it to new head.
"""

# Copied linked list class into file, rotate is new method on line 71
#!python
...
class LinkedList():
    ...
    def rotate(self, k):
        """Rotates linked list by k moves."""
        k %= self.size
        #print(k)
        if k == 0:
            return
...
if __name__ == '__main__':
    ...
    ll = LinkedList([10,20,30,40,50,60])
    print(ll)
    ll.rotate(16)
    print(ll)
